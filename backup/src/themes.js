...
export function declareTheme(el, name, hue, saturation, pastel, defaultMods) {
  ...
  if (!isGlobal) {
    ...
    el.vdsSetContext(key, contextTheme);
  }
...
export function removeTheme(el, name, customProps) {
  ...
  Object.keys(el.vdsContext)
    .forEach(prop => {
      if (prop.startsWith(key)) {
        delete el.vdsContext[prop];
        removeRulesById(el.vdsUniqId, prop);
      }
    });
}
...
export function applyTheme(element, { name, hue, saturation, pastel, type, contrast, lightness }, themeName) {
  ...
  const baseQuery = element === document.body ? 'body' : `#${element.vdsUniqId}`;
  ...
  if (element.vdsSetContext) {
    element.vdsSetContext(`theme:${themeName}`, theme);
  } else {
...
export function generateSchemeCSS(query, [lightNormalProps, lightContrastProps, darkNormalProps, darkContrastProps]) {
  ...
  cssRules.push(
    `html[data-vds-scheme-is="light"][data-vds-contrast-is="no-preference"] ${query} {${lightNormalProps}}`,
    `html[data-vds-scheme-is="dark"][data-vds-contrast-is="no-preference"] ${query} {${darkNormalProps}}`,
    `html[data-vds-scheme-is="light"][data-vds-contrast-is="more"] ${query} {${lightContrastProps}}`,
    `html[data-vds-scheme-is="dark"][data-vds-contrast-is="more"] ${query} {${darkContrastProps}}`,
  );
