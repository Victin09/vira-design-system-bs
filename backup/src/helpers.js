...
const USE_SHADOW = document.querySelector(':root').dataset.vdsShadow != null;
...
export function queryById(element, id, includeNames) {
  ...
  return query(element, `[vds-id="${id}"], [id="${id}"]${includeNames && !id.includes('-') ? `, [vds-${id}]` : ''}`);
}
...
export function error(...args) {
  if (devMode) {
    console.error('vira-design:', ...args);
...
export function generateId(element) {
  if (element.vdsIdGenerated) return element.id;

  element.vdsIdGenerated = true;

  let name = element.id;

  name = name || element.constructor.vdsTag.replace('vds-', '');

  if (name !== 'vds') {
    element.setAttribute('vds-id', name);
  }
  ...
  element.id = id === 1 && name !== 'vds' ? `${name}` : `${name}--${id}`;
...
export const ROOT_STATES_MAP = {
  dev: '[data-vds-dev]',
  dark: '[data-vds-scheme-is="dark"]',
  light: '[data-vds-scheme-is="light"]',
  'high-contrast': '[data-vds-contrast-is="high"]',
  'low-contrast': '[data-vds-contrast-is="low"]',
};
...
function getStateSelector(name, isRoot = false) {
  ...
  return isRoot ? (ROOT_STATES_MAP[name] || `[data-vds-${name}]`) : (STATES_MAP[name] || `[is-${name}]`);
}
...
export function splitStates(attrValue) {
  ...
  if (context) {
    contextStart = CONTEXT_START_MAP[context] || (context.startsWith('#') ? `[vds-id="${context.replace('#', '')}"]` : `[vds-${context}]`);
...
const TASKS = [];
const TASK_EVENT = 'vira-design:task';
...
export function deepQuery(element, selector) {
  if (element.vdsShadow) {
    const shadowEl = deepQuery(element.vdsShadow, selector);
...
export function deepQueryAll(element, selector) {
  ...
  if (element.vdsShadow) {
    found.push(...deepQueryAll(element.vdsShadow, selector));
  }
...
export function getContextOwner(element, name) {
  let context = element.vdsContext;

  while (!context.hasOwnProperty(name)) {
    context = Object.getPrototypeOf(context);
  }

  while (element.vdsContext !== context) {
    element = element.parentNode || element.host;
  }
...
export function getInnerRef(el, name) {
  return el.vdsRefs && el.vdsRefs[name];
}
...
export function setInnerRef(el, name, value) {
  ...
  el.vdsRefs[name] = value;
}

export function removeInnerRef(el, name) {
  if (el.vdsRefs) {
    delete el.vdsRefs[name];
  }
}
...
export function setAria(el, name, value) {
  ...
  if (value == null) {
    (el.vdsRef || el).removeAttribute(`aria-${name}`);
  } else {
    (el.vdsRef || el).setAttribute(`aria-${name}`, value);
  }
}
