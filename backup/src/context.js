...
function setScheme() {
  const setting = ROOT.dataset.vdsScheme;

  ROOT.dataset.vdsSchemeIs = (setting !== 'auto' && SCHEMES.includes(setting) && setting)
    || globalScheme;
}

function setContrast() {
  const setting = ROOT.dataset.vdsContrast;

  ROOT.dataset.vdsContrastIs = (setting !== 'auto' && CONTRASTS.includes(setting) && setting)
    || globalContrast;
}
...
function setOutline() {
  ...
  const showOutline = ROOT.dataset.vdsOutline != null;

  if (showOutline) {
    deepQueryAll(ROOT, '[is-host]')
      .forEach(host => {
        host.vdsSetMod('outline', showOutline);
      });
  }
...
const verifyRoot = asyncDebounce((name) => {
  ...
  requestIdleCallback(() => {
    deepQueryAll(ROOT, '[vds]')
      .forEach(el => el.vdsContextChanged(name));
  });
});
