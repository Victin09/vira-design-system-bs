...
export default class ListBoxBehavior extends WidgetBehavior {
  ...
  init() {
    ...
    this.isPopup = this.host.hasAttribute('vds-popup');

    if (this.isPopup) {
      this.forceLinkValue();
    }

    this.host.vdsListBox = this;
    this.options = [];
  ...
  isEdgeMoveAllowed() {
    ...
    const popup = this.host.vdsPopup;
  ...
  get orderedOptions() {
    ...
    return deepQueryAll(this.host, '[vds-option]:not([disabled])')
