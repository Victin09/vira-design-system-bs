<template>
  <div>
    <div v-if="error" class="vds-alert vds-alert-danger">
      {{ error }}
    </div>
...
import { notification } from "vds";
import { html, parse, openOnCodepen } from "./util";
import { $, $$, attr, offset, on, startsWith, ucfirst } from "vds-util";
...
export default {
            ...
            async (response) => {
              ...
              if (startsWith(response.trim(), "<!DOCTYPE html>")) {
                response = `<h1>Not Found</h1>
                                            <p class="vds-text-lead">We Couldn't Find Your Page!</p>`;
              }
  ...
  methods: {
    setPage(page) {
      ...
      this.$parent.ids = $$('> h2 a[href^="#"]', this.$refs.container).reduce(
        (ids, el) => {
      ...
      setTimeout(() =>
        $$("pre code", this.$refs.container).forEach((block) =>
