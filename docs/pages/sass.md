...
<p class="vds-text-lead">Learn how to modify the Vira Design System styling and create your own theme with Sass.</p>

When you have [installed Vira Design System](installation.md) with sources you will find the Vira Design System Sass version within the _/src/scss_ folder. The Sass version allows you to include customizations in the build process, rather than manually overwriting a lot of CSS rules by hand.

**Note** Sass allows two different syntax version: Sass and SCSS. Vira Design System makes use of the SCSS syntax.

***

## How to build

To include Vira Design System in your project's build workflow, you need to import three SCSS files from Vira Design System in the correct order into in your own SCSS code. Then, compile your file, e.g. running `sass site.scss > site.css` or any other [Sass compiler](http://sass-lang.com/documentation/file.SASS_REFERENCE.html#using_sass). Be careful to keep the correct order as described in the following example.
...
@import "vds/src/scss/variables-theme.scss";
@import "vds/src/scss/mixins-theme.scss";

// 3. Your custom mixin overwrites.
@mixin hook-card() { color: #000; }

// 4. Import ViraDesignSystem.
@import "vds/src/scss/vds-theme.scss";
```

**Note** The example uses the styling of the included default theme. Alternatively, you can import `variables.scss`, `mixins.scss` and `vds.scss` to only include the core styling.

***

## Create a Vira Design System theme

When you have set up a file to put in your own SCSS code, you can get started to theme Vira Design System the way you want. If you have never used SCSS before, check out the [language features](http://sass-lang.com/documentation/file.SASS_REFERENCE.html#features). When working with the Vira Design System SCSS sources, we have a few recommendations.
...
First, find a SCSS variable you want to change inside the Vira Design System source. For example, the global link color is defined in `/src/scss/components/variables.scss`:
...
To prevent overhead selectors, we use Mixins from [Sass](http://sass-lang.com/documentation/file.SASS_REFERENCE.html#mixins), which hook into predefined selectors from the Vira Design System source and inject additional properties. Selectors don't have to be repeated throughout all documents and global changes can be made much more easily.
...
```html
<!-- vds sources, might be in a subfolder when using npm -->
vds/src/scss/
    ...
    <!-- other vds files, some of which we will import below -->
    ...
...
// 2. Import default variables and available mixins.
@import "vds/src/scss/variables.scss";
@import "vds/src/scss/mixins.scss";
...
// 4. Import Vira Design System
@import "vds/src/scss/vds.scss";
```

Now you can compile `site.scss` and the resulting CSS will include all your customizations.

**Note** You can further extend this setup by replacing part "4." with single import statements from the Vira Design System source. You can then omit some components you do not use to produce smaller CSS. Just copy from [src/scss/components/\_import.scss](https://github.com/Victin09/vira-design-system/vds/blob/develop/src/scss/components/_import.scss) and make sure to preserve the correct import order.
