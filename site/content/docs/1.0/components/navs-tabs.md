...
layout: docs
title: Navs and tabs
description: Documentation and examples for how to use Vira Design System's included navigation components.
group: components
...
Navigation available in Vira Design System share general markup and styles, from the base `.nav` class to the active and disabled states. Swap modifier classes to switch between each style.
...
As part of Vira Design System's evolving CSS variables approach, dropdowns now use local CSS variables on `.nav`, `.nav-tabs`, and `.nav-pills` for enhanced real-time customization. Values for the CSS variables are set via Sass, so Sass customization is still supported, too.
...
Use the tab JavaScript plugin—include it individually or through the compiled `vira-design-system.js` file—to extend our navigational tabs and pills to create tabbable panes of local content.
...
triggerTabList.forEach(function (triggerEl) {
  var tabTrigger = new viraDesignSystem.Tab(triggerEl)
...
viraDesignSystem.Tab.getInstance(triggerFirstTabEl).show() // Select first tab
```
...
<script>
  ...
  var firstTab = new viraDesignSystem.Tab(firstTabEl)
...
```js
  ...
  var tab = new viraDesignSystem.Tab(someTabTriggerEl)
...
const tab = viraDesignSystem.Tab.getInstance(triggerEl) // Returns a Vira Design System tab instance
```
...
```js
const triggerEl = document.querySelector('#trigger')
const tab = viraDesignSystem.Tab.getOrCreateInstance(triggerEl) // Returns a Vira Design System tab instance
```
