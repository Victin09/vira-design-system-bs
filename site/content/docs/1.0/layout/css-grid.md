...
Vira Design System's default grid system represents the culmination of over a decade of CSS layout techniques, tried and tested by millions of people. But, it was also created without many of the modern CSS features and techniques we're seeing in browsers like the new CSS Grid.
...
With Vira Design System 5, we've added the option to enable a separate grid system that's built on CSS Grid, but with a Vira Design System twist. You still get classes you can apply on a whim to build responsive layouts, but with a different approach under the hood.
...
- **Columns and gutter sizes are set via CSS variables.** Set these on the parent `.grid` and customize however you want, inline or in a stylesheet, with `--vds-columns` and `--vds-gap`.

In the future, Vira Design System will likely shift to a hybrid solution as the `gap` property has achieved nearly full browser support for flexbox.
...
- Inline and custom styles should be viewed as replacements for modifier classes (e.g., `style="--vds-columns: 3;"` vs `class="row-cols-3"`).
...
- We override the default number of columns with a local CSS variable: `--vds-columns: 3`.
...
{{< example class="bd-example-cssgrid" >}}
<div class="grid" style="--vds-columns: 3;">
  <div>
    First auto-column
    <div class="grid">
      <div>Auto-column</div>
  ...
  <div>
    Second auto-column
    <div class="grid" style="--vds-columns: 12;">
...
| --- | --- | --- |
| `--vds-rows` | `1` | The number of rows in your grid template |
| `--vds-columns` | `12` | The number of columns in your grid template |
| `--vds-gap` | `1.5rem` | The size of the gap between columns (vertical and horizontal) |
{{< /vds-table >}}

These CSS variables have no default value; instead, they apply fallback values that are used _until_ a local instance is provided. For example, we use `var(--vds-rows, 1)` for our CSS Grid rows, which ignores `--vds-rows` because that hasn't been set anywhere yet. Once it is, the `.grid` instance will use that value instead of the fallback value of `1`.
...
{{< example class="bd-example-cssgrid" >}}
<div class="grid" style="--vds-columns: 4; --vds-gap: 5rem;">
...
{{< example class="bd-example-cssgrid" >}}
<div class="grid" style="--vds-columns: 10; --vds-gap: 1rem;">
  <div class="g-col-6">.g-col-6</div>
  <div class="g-col-4">.g-col-4</div>
</div>
{{< /example >}}
...
{{< example class="bd-example-cssgrid" >}}
<div class="grid" style="--vds-rows: 3; --vds-columns: 3;">
  <div>Auto-column</div>
...
Because of that, you can have different vertical and horizontal `gap`s, which can take a single value (all sides) or a pair of values (vertical and horizontal). This can be applied with an inline style for `gap`, or with our `--vds-gap` CSS variable.

{{< example class="bd-example-cssgrid" >}}
<div class="grid" style="--vds-gap: .25rem 1rem;">
  <div class="g-col-6">.g-col-6</div>
  <div class="g-col-6">.g-col-6</div>
...
{{< example class="bd-example-cssgrid" >}}
<div class="grid" style="--vds-columns: 18; --vds-gap: .5rem;">
