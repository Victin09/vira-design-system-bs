...
Here are the variables we include (note that the `:root` is required) that can be accessed anywhere Vira Design System's CSS is loaded. They're located in our `_root.scss` file and included in our compiled dist files.

```css
{{< root.inline >}}
{{- $css := readFile "dist/css/vira-design-system.css" -}}
