...
export default class FixateBehavior {
  ...
  changed(name) {
    ...
    host.vdsSetMod(FIXATE_ATTR, false);
    host.vdsSetContext('fixate', null);
    host.style.display = '';
    host.style.opacity = '';
    delete this.position;

    const fixateValue = host.getAttribute(FIXATE_ATTR)
      || host.constructor.vdsAllStyles[FIXATE_ATTR]
      || 'down';
    ...
    host.vdsSetMod(FIXATE_ATTR, true);
    host.vdsSetContext('fixate', {
      context: host,
  ...
  get enabled() {
    ...
    const defaults = host.constructor.vdsAllStyles;
