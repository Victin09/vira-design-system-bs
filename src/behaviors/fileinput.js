...
export default class FileInputBehavior extends WidgetBehavior {
  ...
  init() {
    ...
    if (existRef) {
      ...
      const container = fixture('<vds-block place="cover" opacity="0" overflow="n"><input/></vds-block>');
    ...
    ref.addEventListener('blur', () => {
      if (host.id) {
        this.emit('vds-blur', host.vdsId, { bubbles: true });
      }
    });
