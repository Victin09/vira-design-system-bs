...
export const BASE_PROPS = VdsEl.vdsPropsList.reduce((props, name) => {
  props[name] = null;
...
export default class WidgetBehavior extends Behavior {
  ...
  setValue(value, silent) {
    ...
    if (!silent) {
      ...
      if (this.host.id) {
        this.emit('vds-change', this.host.vdsId, { bubbles: true });
      }
    }
  ...
  setValidity(bool) {
    ...
    const fieldEl = this.host.closest('[vds-field]');

    if (fieldEl) {
      fieldEl.vdsSetMod('invalid', !bool);
      fieldEl.vdsSetMod('valid', bool);
    }
  }
  ...
  linkHostValue() {
    ...
    if (host.vdsSetValue) {
      const setValue = host.vdsSetValue;

      host.vdsSetValue = (val) => {
        setValue.call(host, val);
        set(val, true);
      };

      if (this.params.primary) {
        host.vdsGetValue = get;
      }
    } else {
      host.vdsSetValue = (val) => set(val, true);
      host.vdsGetValue = get;
    }
  }
  ...
  /**
   * Log anything to vds-debug element.
   * @param args
   */
  log(...args) {
    if (!devMode) return;
