...
export default class ActionBehavior extends WidgetBehavior {
  ...
  init() {
    ...
    this.on('keydown', (event) => {
      if (event.key === 'Escape' && host.vdsHasAria('expanded')) {
        this.set(false);
      }
  ...
  unlinkPopup(popup) {
    ...
    delete this.popup;
    host.vdsSetAria('haspopup', null);
    host.vdsSetAria('expanded', null);
    this.role = this.host.constructor.vdsRole;
  ...
  tap(evt) {
    ...
    if (this.scrollto) {
      host.vdsScrollTo(this.scrollto);
    }
  ...
  set(pressed, silent, force) {
    ...
    if (this.popup) {
      host.vdsSetAria('expanded', pressed);
    } else if (this.isCheckbox()) {
      host.vdsSetAria('checked', pressed);
    } else if (this.isSelectable()) {
      host.vdsSetAria('selected', pressed);
    } else {
      host.vdsSetAria('pressed', pressed);
    }
  ...
  toggleInnerPopup(bool, ignoreMultiple) {
    const innerPopup = this.host.vdsDeepQuery(`[is-popup]${ignoreMultiple ? ':not([multiple])' : ''}`);
    const method = bool == null
      ? (this.pressed ? 'open' : 'close')
      : (bool ? 'open' : 'close');

    if (innerPopup) {
      innerPopup.vdsPopup[method]();
    }
  }
  ...
  isToggle() {
    ...
    return host.hasAttribute('toggle')
      || host.vdsHasAria('pressed')
      || host.vdsHasAria('expanded')
      || host.vdsHasAria('checked')
      || host.vdsHasAria('selected')
