...
export const banner = `/*! ViraDesignSystem ${await getVersion()} | https://www.getvds.com | (c) 2014 - ${new Date().getFullYear()} YOOtheme | MIT License */\n`;
...
export async function compile(file, dest, { external, globals, name, aliases, replaces } = {}) {
    ...
    const outputOptions = {
        ...
        banner,
        format: 'umd',
        amd: { id: `ViraDesignSystem${name}`.toLowerCase() },
        name: `ViraDesignSystem${ucfirst(name)}`,
    ...
    if (!watch) {
        ...
        console.log('ViraDesignSystem is watching the files...');
