...
if (args.h || args.help) {
    console.log(`
        ...
        scope.js // will scope with vds-scope
...
const currentScopeLegacyRe = /\.(vds-scope)/;
...
function getNewScope() {
    const scopeFromInput = args.scope || args.s || 'vds-scope';
    ...
    for (const file of files) {
        await replaceInFile(file, async (data) => {
            ...
            return `/* scoped: ${scope} */\n${
                output.replace(
                    new RegExp(
                        `.${scope}\\s((\\.(vds-(drag|modal-page|offcanvas-page|offcanvas-flip)))|html|:root)`,
                        'g'
                    ),
                    '$1'
                ) // unescape
