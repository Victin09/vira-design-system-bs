...
const sources = [
    { src: 'src/less/vds.less', dist: `dist/css/vds-core${rtl ? '-rtl' : ''}.css` },
    { src: 'src/less/vds.theme.less', dist: `dist/css/vds${rtl ? '-rtl' : ''}.css` },
];
...
for (const src of await glob('custom/*.less')) {
    ...
    const dist = `dist/css/vds.${theme}${rtl ? '-rtl' : ''}.css`;

    themes[theme] = { css: `../${dist}` };

    if (await pathExists(`dist/js/vds-icons-${theme}.js`)) {
        themes[theme].icons = `../dist/js/vds-icons-${theme}.js`;
    }
            ...
            [
                {
                    ...
                    processors: [
                        {
                            expr: ['--vds-position-translate-x', 'stroke-dashoffset'].join('|'),
